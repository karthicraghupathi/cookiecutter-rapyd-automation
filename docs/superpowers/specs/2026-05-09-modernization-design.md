...
├── README.md
├── requirements.txt                     # auto-generated by uv-export pre-commit hook (prod only)
├── requirements-dev.txt                 # auto-generated by uv-export pre-commit hook (prod + dev)
├── uv.lock                              # uv's lockfile; canonical source of truth
├── src/
│   └── {{ project_slug }}/
│       ├── __init__.py
...
- `requirements.txt` — production dependencies only (`--no-dev`). Used in production deploys, CI systems without uv, or any consumer that reads pip-format requirements.
- `requirements-dev.txt` — full set (prod + dev). A developer who clones a fresh checkout can `pip install -r requirements-dev.txt` and have everything needed to run, lint, and test, even without uv installed.
- `--no-hashes` — keeps the files readable; reproducibility is owned by `uv.lock`, not these exports.

These files are generated, but committed: downstream consumers don't always run pre-commit, so the committed copies must stay current. The hooks fail-then-pass on first run after a `pyproject.toml` change (auto-fix behavior) — standard pre-commit flow.
